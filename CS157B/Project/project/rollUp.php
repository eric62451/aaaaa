...
switch ($dimension) {
    case "account":
        if($accountlevel == 2)
		{$sql = "select A.account_state, $branchSQL, $productSQL, $monthSQL, primary_balance from household_facts H, account A, branch B, month M, product P where H.account_key = A.account_key and H.branch_key = B.branch_key and H.month_key = M.month_key and H.product_key = P.product_key group by $branchSQL, $monthSQL, A.account_state, $productSQL;";
			$accountHeader = "Account State";}
    ...
    case "branch":
        if($branchlevel == 2)
		{$sql = "select $accountSQL, B.branch_state, $productSQL, $monthSQL, primary_balance from household_facts H, account A, branch B, month M, product P where H.account_key = A.account_key and H.branch_key = B.branch_key and H.month_key = M.month_key and H.product_key = P.product_key group by B.branch_state, $monthSQL, $accountSQL, $productSQL;";
			$branchHeader = "Branch State";}
    ...
    case "month":
        if($monthlevel == 2)
		{$sql = "select $accountSQL, $branchSQL,$productSQL, M.fiscal_quarter, primary_balance from household_facts H, account A, branch B, month M, product P where H.account_key = A.account_key and H.branch_key = B.branch_key and H.month_key = M.month_key and H.product_key = P.product_key group by $branchSQL, M.fiscal_quarter, $accountSQL, $productSQL;";
			$monthHeader = "Fiscal Quarter";}
		else if($monthlevel == 1)
	...
	case "product":
        if($productlevel == 2)
		{$sql = "select $accountSQL, $branchSQL, P.category, $monthSQL, primary_balance from household_facts H, account A, branch B, month M, product P where H.account_key = A.account_key and H.branch_key = B.branch_key and H.month_key = M.month_key and H.product_key = P.product_key group by $branchSQL, $monthSQL, $accountSQL, P.category;";
			$productHeader = "Product Category";}
